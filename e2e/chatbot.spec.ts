...
test('assistant supports topic exploration and prepares an honest WhatsApp handoff', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Crowns and bridges' }).click()
  await expect(dialog).toContainText('record review')
  ...
  await dialog.getByRole('button', { name: 'Continue' }).click()
  await dialog.getByRole('combobox', { name: 'Service' }).selectOption({ label: 'Crowns & Bridges' })
  await dialog.getByRole('button', { name: 'Continue' }).click()
  ...
  await expect(whatsapp).toHaveAttribute('href', /918390998088/)
