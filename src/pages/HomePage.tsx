  ...
  return <>
    ...
    <section className="service-routes shell" aria-labelledby="service-routes-title">
      ...
      <div className="service-routes__grid">
        <Link aria-label="Explore crowns and bridges" className="service-route service-route--crowns" to="/crowns-bridges">
          <div><span>01</span><p>Crown and bridge laboratory</p><h3>Crowns &amp;<br />bridges</h3>
            <strong>Explore crowns and bridges <ArrowRight aria-hidden size={18} /></strong></div>
        ...
        <Link aria-label="Explore implant restorations" className="service-route service-route--implants" to="/dental-implants">
          <div><span>02</span><p>Implant laboratory support</p><h3>Dental<br />implants</h3>
            <strong>Explore implant restorations <ArrowRight aria-hidden size={18} /></strong></div>
    ...
    <section className="home-workflow" id="workflow" aria-labelledby="home-workflow-title">
      <div className="shell home-workflow__inner">
        <div className="home-workflow__heading"><p className="eyebrow">A visible process</p><h2 id="home-workflow-title">Every case starts with a proper review.</h2></div>
        <ol>{workflow.map(([number, title, copy]) => <li key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p></li>)}</ol>
      </div>
    </section>
    ...
    <section className="home-principles shell" aria-labelledby="principles-title">
      ...
      <div><p>We keep the conversation centred on the prescription, the available records and the requirements of the individual restoration.</p>
        <p>Where a case needs clarification, the right time to raise it is before production — not after the work is complete.</p></div>
    </section>

    <section className="home-closing">
      <div className="shell home-closing__inner"><p>Ready to discuss a case?</p><h2>Start with what you already have.</h2>
        <Link className="button button--lime" to="/contact">Send a case <ArrowRight aria-hidden size={18} /></Link></div>
    </section>
  </>
}
